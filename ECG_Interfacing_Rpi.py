...
#  pip install scipy numpy matplotlib adafruit-circuitpython-ads1x15
#
#  BUGS FIXED IN THIS VERSION
#  ─────────────────────────────────────────────────────────────────────────
#  FIX-1  Plots not saving — root causes:
#    a) plt.tight_layout() called before savefig() on headless Pi with Agg
#       backend can silently fail in some matplotlib versions. Removed all
#       plt.tight_layout() calls. savefig(bbox_inches="tight") handles
#       layout automatically and is the recommended headless approach.
#    b) save_fig() had a try/except that swallowed errors silently.
#       Replaced with explicit error logging + file size verification.
#       Now logs "SAVED 49KB" or "SAVE ERROR: <reason>" for each file.
#    c) Emoji characters (❌ ✅) in monospace text caused Glyph-missing
#       warnings on Pi which in some matplotlib versions abort rendering.
#       Replaced with ASCII: [FAIL] and [OK].
#
#  FIX-2  Status column not showing in summary — root causes:
#    a) N/A rows were formatted WITHOUT a status column — blank by design.
#       Fixed: N/A rows now show "---" in status column.
#    b) Summary image figure was too narrow — status column sometimes
#       rendered outside the visible text bbox area.
#       Fixed: wider figure (12x9.5), smaller font (7.2pt), explicit
#       column widths verified to fit.
#    c) Status for abnormal values was computed correctly but the column
#       header alignment was off. Fixed with consistent column widths.
#
# =============================================================================
...
DATA_SOURCE       = "ads1115"   # "demo" to test without hardware

ADS_I2C_ADDRESS   = 0x48        # ADDR switch LEFT=0x48, RIGHT=0x49
ADS_GAIN          = 1           # 1 → ±4.096 V  (covers 0–3.3 V ECG)
ADS_CHANNEL       = 0           # A0 screw terminal

FS                = 500         # Hz
BUFFER_SECONDS    = 10          # rolling buffer length
LOOP_INTERVAL_SEC = 2.0         # clinical analysis every N seconds
SAVE_EVERY_N_LOOPS= 2           # save plots to disk every N loops

VREF_MID_VOLTS    = 1.65        # ECG circuit mid-rail (V)
                                 # Measure with multimeter on your circuit
                                 # If ECG output is already centred on 0V, set 0.0
VOLTS_TO_MV       = 1000.0
NOTCH_FREQ_HZ     = 50.0        # 50 Hz India/EU  |  60 Hz North America

TCP_HOST          = "0.0.0.0"   # listen on all interfaces
TCP_PORT          = 9000
STREAM_DECIMATE   = 5           # 500/5 = 100 samples/sec sent to laptop

OUTPUT_BASE_DIR   = os.path.expanduser("~/Downloads/ECG")

SNR_POOR_DB       = 10.0        # SNR below this = poor signal warning
...
# =============================================================================
#  CSV LIVE LOGGER
#  One file per session, records every sample from start to Ctrl+C.
#  Use _last_t watermark so no sample is ever written twice.
# =============================================================================
...
class CSVLogger:
    ...
    def write(self, t_arr, raw_arr, filtered_arr, r_peaks, loop_idx, hr, snr_db):
        """Append only new samples (timestamp > _last_t).  No duplicates."""
        # Build R-peak time set for fast lookup
        rp_set = set(t_arr[r_peaks].tolist()) if len(r_peaks) else set()
        tol    = 1.5 / FS

        # Mask: only samples not yet written
        mask    = t_arr > self._last_t
        new_t   = t_arr[mask]
        new_raw = raw_arr[mask]
        new_flt = filtered_arr[mask]
        if len(new_t) == 0:
            return
...
class DemoSource:
    """Synthetic P-QRS-T ECG with mild HR drift. Use for testing without hardware."""
    ...
    def _make_beat(fs, bpm):
        rr=60.0/bpm; n=int(rr*fs)
        t=np.linspace(0.0, rr, n, endpoint=False)
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            with self._lock:
                self._buf.extend(zip(times.tolist(), chunk.tolist()))
...
class ADS1115Source:
    """
    Reads DFRobot Gravity ADS1115 via I2C in continuous conversion mode.
    Compatible with both old and new adafruit_ads1x15 library versions.
    """
    ...
    def __init__(self, address=ADS_I2C_ADDRESS, gain=ADS_GAIN,
        ...
        # Handle different library versions for Mode enum
        try:
        ...
        ads.data_rate = best
        log.info(f"ADS1115  0x{address:02X}  ±{4.096/gain:.3f}V  {best}SPS  A{channel}")

        chan_map = {0:ADS_MODULE.P0, 1:ADS_MODULE.P1,
                   2:ADS_MODULE.P2, 3:ADS_MODULE.P3}
        self._chan = AnalogIn(ads, chan_map[channel])
...
def bandpass_filter(sig, fs):
    """0.5–40 Hz Butterworth bandpass — removes baseline wander and EMG."""
...
def compute_snr(raw, filtered):
    ...
    noise = raw - filtered  (everything the bandpass removed)
    >20 dB = good,  10-20 dB = moderate,  <10 dB = poor
    """
    noise  = raw - filtered
    s_pwr  = float(np.mean(filtered**2))
    n_pwr  = float(np.mean(noise**2))
    if n_pwr < 1e-12: return 99.0
...
def detect_r_peaks(filtered, fs):
    ...
    Key fix: uses argmax(|seg|) instead of polarity-based argmax/argmin
    so it works whether median(filtered) is slightly positive or negative.
    Adaptive threshold fallback for noisy real-world signals.
    """
    if len(filtered) < int(0.6*fs):
    ...
    # Refine to exact R-peak location (wider window to handle envelope lag)
    hw=max(1, int(0.10*fs))
    refined=[]
    for p in pks:
        s=max(0,p-hw); e=min(len(filtered),p+hw); seg=filtered[s:e]
        if len(seg)==0: continue
        refined.append(s + int(np.argmax(np.abs(seg))))  # abs() = polarity-safe
...
def _pick(segs):
    """Return the lowest-noise representative beat segment."""
    return segs[int(np.argmin([np.sum(w**2) for w in segs]))] if segs else None


def extract_features(sig, r_peaks, t_arr, fs):
    """Extract all clinical scalar features from filtered signal + R-peaks."""
    f={}
    ...
    if repT is not None:
        pk=int(np.argmax(np.abs(repT)))
        f["T_amp"]=float(repT[pk])   # signed — negative = inverted T
    ...
    # Waveform segments for plots — never sent over TCP
...
def _nn(v):
    """True if v is a real finite number (not None, not NaN)."""
    return v is not None and not (isinstance(v, float) and np.isnan(v))


def detect_abnormalities(feats, snr_db=None):
    abn=[]; g=lambda k: feats.get(k, np.nan); R=ECG_REF

    if snr_db is not None and snr_db < SNR_POOR_DB:
        abn.append(f"Poor signal quality  SNR={snr_db:.1f}dB (check electrodes/contacts)")
    ...
    if _nn(PR):
        if   PR < R["PR"]["duration"][0]: abn.append("Short PR interval  (WPW / pre-excitation)")
        elif PR > R["PR"]["duration"][1]: abn.append("Prolonged PR  (1st-degree AV block)")
    if _nn(QRS_d) and QRS_d > R["QRS"]["duration"][1]:
        abn.append("Wide QRS complex  (BBB / ventricular rhythm)")
    if _nn(QRS_a):
...
#  FIX: status column on EVERY row, ASCII only (no emoji), correct formatting
# =============================================================================

def _fmt_v(v):
    """Format a range boundary value with appropriate decimal places."""
    if v is None: return "---"
    av=abs(v)
    if av==0:   return "0"
...
def _get_status(disp_val, nrange):
    """
    Returns status string for a feature value against its normal range.
    disp_val is already in display units (ms or mV as appropriate).
    nrange is (lo, hi) in the same display units.
    """
    if nrange is None:
        return "---"
    lo, hi = nrange
...
def build_summary_text(feats, abn, loop_idx, snr_db=None):
    """
    Build the clinical summary text string.
    FIX-2: Every row has a status column.
    ASCII only — no emoji (Pi monospace font compatibility).
    Columns: Feature | Value | Unit | Normal Range | Status
    """
    ts = time.strftime("%Y-%m-%d  %H:%M:%S")

    # (display_name, feature_key, unit, multiply_1000_for_ms, (lo,hi) in display units)
    rows = [
    ...
    #       col:  Feature(20) | Value(8) | Unit(4) | Range(13) | Status(10)
    ...
    lines = [
        "=" * 68,
        "  ECG REAL-TIME CLINICAL SUMMARY",
        f"  Loop {loop_idx:04d}     {ts}",
        "=" * 68,
        "",
        HDR,
        SEP,
    ...
    for name, key, unit, to_ms, nrange in rows:
        ...
        if not _nn(raw):
            # FIX-2a: N/A rows now include the normal range AND "N/A" in status
            nr = (f"{_fmt_v(nrange[0])}-{_fmt_v(nrange[1])}"
                  if nrange else "---")
            lines.append(
                f"  {name:<20}  {'N/A':>8}  {unit:<4}  {nr:<13}  {'N/A':<10}")
            continue

        disp = raw * 1000.0 if to_ms else raw
        nr   = (f"{_fmt_v(nrange[0])}-{_fmt_v(nrange[1])}"
                if nrange else "---")
        # FIX-2b: status always computed and always shown
        status = _get_status(disp, nrange)

        lines.append(
            f"  {name:<20}  {disp:>8.2f}  {unit:<4}  {nr:<13}  {status:<10}")

    # SNR row
    lines.append(SEP)
    if snr_db is not None:
        snr_str    = f"{snr_db:.1f}"
        snr_status = ("GOOD" if snr_db >= 20
                      else "MODERATE" if snr_db >= 10
                      else "POOR")
    else:
        snr_str = "N/A"; snr_status = "N/A"

    lines.append(
        f"  {'Signal SNR':<20}  {snr_str:>8}  {'dB':<4}  {'>20=good':<13}  {snr_status:<10}")

    # Findings section — ASCII only (no emoji — Pi font compatibility)
    lines += [
        "",
        "  CLINICAL FINDINGS",
        SEP,
...
def _style_ax(ax, title, xl="", yl=""):
    ...
    for sp in ax.spines.values():
        sp.set_edgecolor("#30363d"); sp.set_linewidth(0.5)


def _spectrum(sig, fs):
    freqs = rfftfreq(len(sig), 1.0/fs)
    mag   = np.abs(rfft(sig)) / len(sig)
    return freqs, mag, mag**2


def _save_figure(fig, filepath: Path) -> str:
    """
    FIX-1: Save figure to disk using savefig(bbox_inches='tight') — NO tight_layout().
    bbox_inches='tight' handles the layout in a single step and is
    reliable on headless Agg backends.
    Verifies the file was actually written (size > 0).
    Returns base64 string for TCP transmission, or "" on failure.
    """
    try:
        fig.savefig(
            str(filepath),
            dpi         = 110,
            bbox_inches = "tight",          # handles layout — replaces tight_layout()
        ...
        if not filepath.exists():
            log.error(f"  File not created: {filepath.name}")
            return ""
        sz = filepath.stat().st_size
        if sz == 0:
            log.error(f"  File is 0 bytes: {filepath.name}")
            return ""
        ...
        log.error(f"  SAVE ERROR for {filepath.name}: {e}")
...
#  Each plot is:  build figure → savefig to disk → read bytes → base64 for TCP
#  NO plt.tight_layout() before save — bbox_inches='tight' handles this.
# =============================================================================

def build_and_save_plots(t_arr, raw, filtered, r_peaks, feats, abn,
                         loop_idx, fs, snr_db, loop_folder: Path):
    """
    Builds all plots, saves each to loop_folder immediately after creation,
    and returns base64 strings for TCP transmission.

    Returns: (plots_b64 dict, plot_names list, summary_text string)
    """
    plt.rcParams.update(_DARK)

    plots_b64  = {}   # { tcp_key: base64_string }
    plot_names = []   # [ filename_stem, ... ]
    txt        = build_summary_text(feats, abn, loop_idx, snr_db)

    def _save(fig, tcp_key, stem):
        """Save figure, register in output dicts."""
        path = loop_folder / f"{stem}.png"
        b64  = _save_figure(fig, path)
        if b64:
            plots_b64[tcp_key] = b64
            plot_names.append(stem)

    hr_txt  = feats.get("HR", np.nan)
    hr_s    = f"   HR:{hr_txt:.0f}bpm" if _nn(hr_txt) else "   HR:---"
    snr_s   = f"   SNR:{snr_db:.1f}dB" if snr_db is not None else ""
    n_rp    = len(r_peaks)
    ...
    if n_rp:
        ax.scatter(t_arr[r_peaks], filtered[r_peaks],
                   color="#f85149", s=18, zorder=5, label=f"R-peaks ({n_rp})")
        ax.legend(fontsize=7, facecolor="#161b22", edgecolor="#30363d",
                  labelcolor="#c9d1d9")
    _style_ax(ax,
              f"Filtered ECG  Loop {loop_idx}{hr_s}{snr_s}  [{n_rp} R-peaks]",
              "Time (s)", "Amplitude (mV)")
    _save(fig, "ecg", "ECG_Filtered")

    # ── 2. Raw vs Filtered ────────────────────────────────────────────────
    fig, (ax0, ax1) = plt.subplots(2, 1, figsize=(13, 5),
    ...
    if n_rp:
        ax1.scatter(t_arr[r_peaks], filtered[r_peaks],
                    color="#f85149", s=14, zorder=5)
    _style_ax(ax1,
              f"Filtered (0.5-40Hz bandpass + {NOTCH_FREQ_HZ:.0f}Hz notch)",
              "Time (s)", "Amplitude (mV)")
    snr_title = (f"Raw vs Filtered   SNR: {snr_db:.1f} dB"
                 if snr_db is not None else "Raw vs Filtered")
    fig.suptitle(snr_title, color="#58a6ff", fontsize=10)
    _save(fig, "raw_vs_filt", "Raw_vs_Filtered")

    # ── 3. Full Signal FFT ────────────────────────────────────────────────
    freq_arr, mag_arr, pwr_arr = _spectrum(filtered, fs)
    mask = freq_arr <= 80
    fig, (ax_m, ax_p) = plt.subplots(1, 2, figsize=(11, 3.5),
                                      facecolor="#0d1117")
    ax_m.plot(freq_arr[mask], mag_arr[mask], color="#a371f7", lw=0.8)
    _style_ax(ax_m, "FFT Magnitude (0-80 Hz)", "Hz", "Magnitude")
    ax_p.semilogy(freq_arr[mask], pwr_arr[mask]+1e-20, color="#f0883e", lw=0.8)
    _style_ax(ax_p, "FFT Power Spectrum", "Hz", "Power")
    fig.suptitle(f"Full Signal FFT  Loop {loop_idx}", color="#58a6ff", fontsize=10)
    _save(fig, "fft", "FFT_Full_Signal")

    # ── 4. Welch PSD ──────────────────────────────────────────────────────
    npg = min(1024, max(8, len(filtered)//2))
    fw, psd = welch(filtered, fs, nperseg=npg)
    ...
    for feat_key, title, clr, tcp_key, stem in wave_specs:
        seg = feats.get(feat_key)
        if seg is not None and len(seg) > 4:
            ...
            _style_ax(ax, f"{title}  Loop {loop_idx}", "Hz", "Magnitude")
            _save(fig, tcp_key, stem)
    ...
    if repST is not None and len(repST) > 2:
        ...
        if _nn(stv):
            ax.axhline(stv, color="#f0883e", ls="--", lw=1,
                       label=f"Mean ST: {stv:+.3f}mV")
        ax.axhline( 0.1, color="#f85149", ls=":", lw=0.8, alpha=0.7,
                   label="+0.1mV (elevation)")
        ax.axhline(-0.1, color="#79c0ff", ls=":", lw=0.8, alpha=0.7,
                   label="-0.1mV (depression)")
        ax.axhline(0, color="#8b949e", ls="-", lw=0.4, alpha=0.4)
        ax.legend(fontsize=7, facecolor="#161b22", edgecolor="#30363d",
                  labelcolor="#c9d1d9")
        _style_ax(ax, f"ST Segment  Loop {loop_idx}", "Time (ms)", "Amplitude (mV)")
        _save(fig, "st", "ST_Segment")

    # ── 9. RR Tachogram ──────────────────────────────────────────────────
    rr_arr = feats.get("_rr")
    if rr_arr is not None and len(rr_arr) >= 2:
        fig, ax = plt.subplots(figsize=(9, 3.2), facecolor="#0d1117")
        ax.plot(np.arange(1, len(rr_arr)+1), rr_arr*1000,
                "o-", color="#79c0ff", ms=3.5, lw=0.9, rasterized=True)
        ax.axhline(600,  color="#f85149", ls="--", lw=0.8, alpha=0.6,
                   label="100 bpm limit")
        ax.axhline(1000, color="#3fb950", ls="--", lw=0.8, alpha=0.6,
                   label="60 bpm limit")
        ax.legend(fontsize=7, facecolor="#161b22", edgecolor="#30363d",
                  labelcolor="#c9d1d9")
        rmssd = feats.get("RMSSD", np.nan)
        rs    = f"   RMSSD:{rmssd*1000:.1f}ms" if _nn(rmssd) else ""
        _style_ax(ax, f"RR Tachogram  Loop {loop_idx}{rs}",
                  "Beat number", "RR interval (ms)")
        _save(fig, "rr", "RR_Tachogram")

    # ── 10. Clinical Summary Image ────────────────────────────────────────
    # FIX-1: wider figure + smaller font so status column always fits
    # FIX-2: summary text now has status on every row (built by build_summary_text)
    ...
    ax.text(
        ...
        bbox       = dict(boxstyle="round", facecolor="#0d1117",
                          edgecolor="#30363d", alpha=1.0),
    )
    fig.suptitle(f"Clinical Summary  Loop {loop_idx}",
                 fontsize=12, fontweight="bold", color="#f85149")
    _save(fig, "summary", "Clinical_Summary")

    # Save text summary separately
    (loop_folder / "clinical_summary.txt").write_text(txt, encoding="utf-8")

    # Count saved files
    n_png = len(list(loop_folder.glob("*.png")))
    log.info(f"Loop {loop_idx} complete  →  {loop_folder.name}"
             f"  ({n_png} PNG files + clinical_summary.txt)")

    plots_b64["summary_txt"] = txt   # send text to laptop as well
...
class TCPServer:
    def __init__(self, host=TCP_HOST, port=TCP_PORT):
        ...
        self._stream_dec  = 0
        self._send_q: deque = deque(maxlen=200)
        log.info(f"TCP server listening  {host}:{port}")
        threading.Thread(target=self._accept_loop, daemon=True,
                         name="TCPAccept").start()
...
def run(source, tcp: TCPServer, session_folder: Path, csv_log: CSVLogger):
    ...
    t_buf: deque = deque(maxlen=buf_size)
    x_buf: deque = deque(maxlen=buf_size)
    loop_idx = 0
    ...
    while True:
        # Drain source → rolling buffer + live stream to laptop
        for tv, av in source.read():
            t_buf.append(tv); x_buf.append(av); tcp.send_stream(tv, av)

        now = time.monotonic()
        if (now - t_last) < LOOP_INTERVAL_SEC:
            time.sleep(0.015); continue
        t_last = now; loop_idx += 1

        # Need at least 3 s of data before first analysis
        ...
        # ── DSP ──────────────────────────────────────────────────────────
        try:
            filtered = notch_filter(bandpass_filter(raw, FS), FS)
        except Exception as e:
            log.warning(f"Filter error: {e}"); filtered = raw.copy()

        # ── Analysis ─────────────────────────────────────────────────────
        snr_db  = compute_snr(raw, filtered)
        r_peaks = detect_r_peaks(filtered, FS)
        feats   = extract_features(filtered, r_peaks, t_arr, FS) \
                  if len(r_peaks) >= 2 else {}
        feats["SNR_dB"] = snr_db
        abn     = detect_abnormalities(feats, snr_db)

        # -- CSV: write only NEW samples this loop (watermark prevents duplicates)
        csv_log.write(t_arr, raw, filtered, r_peaks, loop_idx,
                      feats.get("HR", None), snr_db)
        ...
        log.info(f"Loop {loop_idx:04d}  HR:{hr_s}  ST:{st_s}  QTc:{qtc_s}"
                 f"  SNR:{snr_db:.1f}dB  Rpeaks:{len(r_peaks)}{flag}")
        ...
        if do_save:
            ...
            try:
                plots_b64, plot_names, txt = build_and_save_plots(
                    t_arr, raw, filtered, r_peaks,
                    feats, abn, loop_idx, FS, snr_db, lf)
            except Exception as e:
                log.error(f"Plot/save error: {e}")
                import traceback; traceback.print_exc()
                plots_b64, plot_names, txt = {}, [], \
                    build_summary_text(feats, abn, loop_idx, snr_db)
        else:
            txt       = build_summary_text(feats, abn, loop_idx, snr_db)
            plots_b64 = {}
            plot_names = []
...
def main():
    ...
    _test = out / "_write_test.tmp"
    try:
        _test.write_text("ok"); _test.unlink()
        log.info(f"Output directory OK: {out}")
    except Exception as e:
        log.error(f"Cannot write to output directory: {out}")
        log.error(f"Reason: {e}")
        log.error("Fix OUTPUT_BASE_DIR in USER CONFIGURATION and restart.")
        sys.exit(1)
    ...
    if DATA_SOURCE == "ads1115":
        try:
            source = ADS1115Source()
            log.info("ADS1115 source ready")
        except Exception as e:
            log.error(f"ADS1115 init failed: {e}")
            log.warning("Falling back to DEMO mode")
            source = DemoSource()
    else:
        log.info("DEMO mode — synthetic ECG signal")
        source = DemoSource()

    tcp     = TCPServer()
    csv_log = CSVLogger(session_folder)  # one file for the whole session

    # ── Print laptop command ───────────────────────────────────────────────
    try:
    ...
    def _shutdown(sig_num, frame):
        ...
        source.stop(); tcp.stop()
        csv_log.close()  # flush and close CSV
        print(f"\n  Results saved in:\n  {session_folder}\n")
        sys.exit(0)
